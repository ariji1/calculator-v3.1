...
//		}
	...
	public boolean seterrOperand1(String value) 
	{
//		if (!setsqrtOperand(value)) // to call the new setter for square root calculation
//		{
//			return true;
...
//		return result.toString(); 
		double i2 = Double.parseDouble(err_result.toString()); // this casts the result to string using the toString() method and then we cast it to Double again to use round operation
		return Double.toString(Math.round(i2*10000000000.0)/10000000000.0); // this cast is used to round off to 10 decimal places, we can extend or decrease the zeros as per the precision req.
	}
...
//		return result.toString(); // else clause of this conditional block
	...
	{
		err_result = new CalculatorValue(err_operand1);
		err_result.div(err_operand2);
		err_resultErrorMessage = err_result.getErrorMessage();
		if (err_resultErrorMessage.length() != 0) // a very intuitive approach to handle divide by zero approach was to use length of error message as the cue
		{
			return "";// since the condition returns empty, another conditional is set up in the Userinterface class to display the error
		}
//		return result.toString(); // else clause of this conditional block
